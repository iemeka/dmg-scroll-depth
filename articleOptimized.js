(function () {
	...
	const config = {
		...
		testVariation: "Article optimized:",
	};
	...
	function createProgressBar() {
		...
		bar.classList.add("augustine-scroll-progress-bar");
		document.body.appendChild(bar);

		// css for progress bar
		const style = document.createElement("style");
		style.textContent = `
        .augustine-scroll-progress-bar {
            --scroll: 0%;
	...
	function scrollProgress() {
		try {
			...
			const otherHeight =
				(document.getElementById("post-extra")?.scrollHeight ?? 0) +
				(document.getElementById("metro-comments-area")?.scrollHeight ?? 0) +
				(document.getElementById("widget-area-home-2")?.scrollHeight ?? 0) +
				(document.querySelector(".metro-infinite-feed-container")?.scrollHeight ??
					0) +
				(document.getElementById("page-bottom-container")?.scrollHeight ?? 0);

			const scrollPosition = root.scrollTop;
			const scrollableArea = root.scrollHeight - root.clientHeight - otherHeight;
			let scrollPercent = (scrollPosition / scrollableArea) * 100;
